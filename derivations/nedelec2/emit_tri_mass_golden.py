...
R2 surface basis that `crates/rapidfem-fd/src/tri_assembly.rs::tri_stiff`
implements, integrated exactly via the barycentric area identity

    ∫_tri L_a^p L_b^q L_c^r dA = 2·p!·q!·r!/(p+q+r+2)! · Area .

The emitted cargo test calls the real Rust `tri_stiff` for several test
...
# Surface local edge order, must match tri_assembly.rs::TRI_EDGE_MAP.
TRI_EDGE_MAP = [(0, 1), (1, 2), (0, 2)]
...
def tri_mass_matrix(verts3, gamma):
    """8×8 symbolic γ·∫ φ_i·φ_j dA, mirroring tri_stiff exactly."""
...
// basis (exact barycentric area integration). Pins tri_stiff entrywise.
